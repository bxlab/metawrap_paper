...
plot_bin_set <-function (file, out) {
  ...
  colnames(orig)[4] <- "Bin membership"
  ...
  g<-g +
    ...
    scale_y_log10(limits=c(5, 2000),breaks = c(1,10,100,1000)) + scale_x_continuous(limits=c(0.3, 0.8),breaks = seq(0,1,.2)) +
...
plot_data_set <-function (file, out, taxlevel) {
  arg_input_file <- file
  arg_ignore_below_prop=0.0005
  arg_taxlevel=taxlevel

  orig <- read.delim(arg_input_file,header=TRUE,sep="\t")
  # change culumn name to be more informative 
  if (taxlevel!="binned_phylum") {
	  colnames(orig)[4] <- "All contigs"
  } else {
	  colnames(orig)[4] <- "Binned contigs"
  }
  ...
  taxa <- c("Not annotated", "Euryarchaeota", "Chlorophyta", "Actinobacteria", "Cyanobacteria", "Bacteroidetes", "Proteobacteria", "Other")
  colors <- paultol[[8]]   
...
# plot phylum
g1 <- plot_data_set("Halite.blobplot", args[1], "taxlevel_phylum")
g2 <- plot_data_set("Halite.blobplot", args[1], "binned_phylum")
g3 <- plot_bin_set("Halite.blobplot", args[1])
...
top_row <- grid.arrange(g1, g2, g3, ncol=3, widths=c(2, 1.7, 1.9))
legend_row <- grid.arrange(legend, blank, ncol=2, widths=c(1, 0.7))
