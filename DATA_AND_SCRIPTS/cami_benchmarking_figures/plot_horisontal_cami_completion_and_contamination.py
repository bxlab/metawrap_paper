...
set_list=["metaWRAP", "Binning_refiner", "DAS_Tool", "metaBAT2", "MaxBin2", "CONCOCT"]
data_sets=["CAMI_high", "CAMI_medium", "CAMI_low"]
...
for data_set in data_sets:
    ...
    for file_name in set_list:
        ...
        for line in open(data_set+"/"+file_name+".stats"):
            ...
            if 1-float(line.split("\t")[1])>1.0*max_contamination/100: continue
    ...
    if pos==3: 
        legend=plt.legend(bbox_to_anchor=(1.1, 0.4), ncol=1, facecolor=None, prop={'size': 20}, frameon=False, title="Original binners", columnspacing=0.5)
...
for data_set in data_sets:
    ...
    for file_name in set_list:
        ...
        for line in open(data_set+"/"+file_name+".stats"):
            ...
            data[bin_set].append(100-100*float(line.split("\t")[1]))
    ...
    if pos==4: plt.text(-0.18*max_x, 5, "Contamination %", fontsize=30, ha="center", rotation='vertical', verticalalignment='center')
        ...
        legend=plt.legend(bbox_to_anchor=(1.1, 1), ncol=1, facecolor=None, prop={'size': 20}, frameon=False, title="Bin refiners", columnspacing=0.5)
        legend.get_title().set_fontsize('24')
        legend._legend_box.align = "left"
...
plt.subplots_adjust(top=0.93, right=0.8, left=0.08, bottom=0.08)
