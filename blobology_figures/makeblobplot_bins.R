...
cov_colnames=colnames(orig)[grep("Metagenome",colnames(orig))]
...
for (t in levels(mfilt[,taxlevel])) {
  g <- g + geom_point(data=mfilt[mfilt[,taxlevel]==t,],aes_string(x="gc", y="cov", colour=taxlevel), size=2, alpha=I(1/3), show.legend=T)
}

g<-g +
  facet_wrap(~read_set, ncol=numcols) + 
  scale_y_log10(limits=c(5, 1600),breaks = c(1,10,100,1000,10000)) + scale_x_continuous(limits=c(0.2, 0.8),breaks = seq(0,1,.1)) +
#  scale_y_log10() + scale_x_continuous(limits=c(0, 1),breaks = seq(0,1,.01)) +
